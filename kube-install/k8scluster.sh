...
createS3Bucket() {
	...
	aws s3 rb s3://$SUBDOMAIN_NAME
	aws s3 mb s3://$SUBDOMAIN_NAME  --region us-west-2
	echo "######### export KOPS_STATE_STORE ######"
	export KOPS_STATE_STORE=s3://$SUBDOMAIN_NAME
...
createSubDomain() {
	...
	ID=$(uuidgen) && aws route53 create-hosted-zone --name $SUBDOMAIN_NAME --vpc VPCId=vpc-5053ca29,VPCRegion=us-west-2 --caller-reference $ID >>$HOSTED_ZONE_FILE
}
